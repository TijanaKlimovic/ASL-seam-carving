...
void calculate(int width, int height, int T_width, int T_height, int width_diff, struct cell_T *T) {
	...
	if (T_height == 0) {
		...
		T[T_index].i = (unsigned char *)malloc(3 * image_height * (image_width - 1)
		...
		for (k = 0; k < image_height; ++k) { // construct each row at a time
			...
			for (l = 0; l < image_width; ++l) {
				if (l != backtrack[k]) { // check for elem to remove
					// remove R 
					T[T_index].i[k*(image_width-1)*3 + crr_col*3] =
						T[T_index_left].i[k*image_width*3 + l*3];
					// remove G
					T[T_index].i[k*(image_width-1)*3 + crr_col*3 + 1] =
						T[T_index_left].i[k*image_width*3 + l*3 + 1];
					// remove B
					T[T_index].i[k*(image_width-1)*3 + crr_col*3 + 2] =
						T[T_index_left].i[k*image_width*3 + l*3 + 2];
					crr_col++;
		...
		T[T_index].i = (unsigned char *)malloc(3 * (image_height - 1) * image_width
			* sizeof(unsigned char));
		...
		for (k = 0; k < image_width; ++k) { // construct each column at a time
			...
			for (l = 0; l < image_height; ++l) {
				if (l != backtrack[k]) { // check for elem to remove
					// remove R
					T[T_index].i[crr_row*image_width*3 + k*3] =
						T[T_index_up].i[l*image_width*3 + k*3];
					// remove G
					T[T_index].i[crr_row*image_width*3 + k*3 + 1] =
						T[T_index_up].i[l*image_width*3 + k*3 + 1];
					// remove B
					T[T_index].i[crr_row*image_width*3 + k*3 + 2] =
						T[T_index_up].i[l*image_width*3 + k*3 + 2];
					crr_row++;
		...
		int optimal_cost_left = min_seam(image_left_height, image_left_width, image_left,
			1, backtrack_left);
		...
		if (T[T_index_left].optimal_cost + optimal_cost_left <=
			...
			T[T_index].i = (unsigned char *)malloc(3 * image_left_height *
				(image_left_width - 1) * sizeof(unsigned char));
			...
			for (k = 0; k < image_left_height; ++k) { // construct each row at a time
				...
				for (l = 0; l < image_left_width; ++l) {
					if (l != backtrack_left[k]) { // check for elem to remove
						// remove R
						T[T_index].i[k*(image_left_width-1)*3 + crr_col*3] =
							T[T_index_left].i[k*image_left_width*3 + l*3];
						// remove G
						T[T_index].i[k*(image_left_width-1)*3 + crr_col*3 + 1] =
							T[T_index_left].i[k*image_left_width*3 + l*3 + 1];
						// remove B
						T[T_index].i[k*(image_left_width-1)*3 + crr_col*3 + 2] =
							T[T_index_left].i[k*image_left_width*3 + l*3 + 2];
						crr_col++;
		...
		// remove row
			...
			T[T_index].i = (unsigned char *)malloc(3 * (image_up_height - 1)
				* image_up_width * sizeof(unsigned char));
			...
			for (k = 0; k < image_up_width; ++k) { // construct each column at a time
				...
				for (l = 0; l < image_up_height; ++l) {
					if (l != backtrack_up[k]) { // check for elem to remove
						// remove R
						T[T_index].i[crr_row*image_up_width*3 + k*3] =
							T[T_index_up].i[l*image_up_width*3 + k*3];
						// remove G
						T[T_index].i[crr_row*image_up_width*3 + k*3 + 1] =
							T[T_index_up].i[l*image_up_width*3 + k*3 + 1];
						// remove B
						T[T_index].i[crr_row*image_up_width*3 + k*3 + 2] =
							T[T_index_up].i[l*image_up_width*3 + k*3 + 2];
						crr_row++;
...
unsigned char *optimal_image(int width, int height, int width_diff,
	...
	if (height_diff >= 3) {
		...
		// copy 
		unsigned char *res = malloc(3 * (width - width_diff + 1)
			* (height - height_diff + 1) * sizeof(unsigned char));
		memcpy(res, T[width_diff * height_diff - 1].i, 3 * (width - width_diff + 1)
			* (height - height_diff + 1) * sizeof(unsigned char));
